...
interface cpu_apb_interface(input pclk);
    ...
    //____________________________________________________READ & WRITE COVERS
      ...
      //   and lv2_rd responses in the same cycle they are generated, so pready is
      //   always 1 in ACCESS state. These covers are structurally unreachable.
      ...
      //   Require two consecutive complete APB transfers without an IDLE cycle.
      //   The DUT always returns to IDLE between independent L2 transactions.
      //   These covers are structurally unreachable.
      ...
      //   Require back-to-back transfers of alternating direction without IDLE.
      //   Same reason as burst covers — structurally unreachable.
      ...
      //   Requires ACCESS followed immediately by SETUP (no IDLE). The DUT
      //   always transitions ACCESS→IDLE→SETUP, never ACCESS→SETUP directly.
      //   Structurally unreachable — commented out.
      //   cover_setup_after_access_state: cover property (access_state ##1 setup_state);
    ...
    covergroup signal_values_cg @(posedge pclk);
       ...
       cp_paddr : coverpoint paddr {
         ...
         // generated by the CPU driver. Use ranges to capture observed traffic.
