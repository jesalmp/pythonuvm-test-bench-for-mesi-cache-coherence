...
module main_func_lv1_dl #(
    ...
    always @(posedge clk) begin
        ...
        // proc side
        if(cpu_rd && blk_hit_proc && !(bus_lv1_lv2_gnt_snoop && (addr_bus_lv1_lv2[ADDR_WID - 1 : 2] === addr_bus_cpu_lv1[ADDR_WID - 1 : 2]))) begin
            data_bus_cpu_lv1_reg   <= cache_var[{index_proc,blk_access_proc}];
            data_in_bus_cpu_lv1_dl <= 1'b1;
            blk_accessed_main      <= blk_access_proc;
        end
        // read miss
        else if(cpu_rd && !blk_hit_proc && !(bus_lv1_lv2_gnt_snoop && (addr_bus_lv1_lv2[ADDR_WID - 1 : 2] === addr_bus_cpu_lv1[ADDR_WID - 1 : 2]))) begin
            ...
            // replacement
            else if(!blk_free && bus_lv1_lv2_gnt_proc) begin
	       $display("CPU_RD LV1 block %0d replacement for address %0x ", blk_access_proc, {`CACHE_CURRENT_TAG_PROC,index_proc,2'b00});
        ...
        //proc write
        if(cpu_wr && !(bus_lv1_lv2_gnt_snoop && (addr_bus_lv1_lv2[ADDR_WID - 1 : 2] === addr_bus_cpu_lv1[ADDR_WID - 1 : 2]))) begin
            bus_lv1_lv2_req_proc_dl <= 1'b1;
            if(bus_lv1_lv2_gnt_proc) begin
                bus_rd_reg           <= 1'b0;
                invalidate_reg       <= 1'b0;
            ...
            // replacement
            else if(bus_lv1_lv2_gnt_proc) begin
	       $display("CPU_WR LV1 block %0d replacement for address %0x ", blk_access_proc, {`CACHE_CURRENT_TAG_PROC,index_proc,2'b00});
                ...
                case (`CACHE_CURRENT_MESI_PROC)
                    ...
                    MODIFIED: begin
                        if(bus_lv1_lv2_gnt_proc) begin
                            addr_bus_lv1_lv2_reg <= {cache_proc_contr[{index_proc,lru_replacement_proc}]
                                [CACHE_TAG_MSB:CACHE_TAG_LSB],index_proc,2'b00};
                            lv2_wr               <= 1'b1;
                            data_bus_lv1_lv2_reg <= cache_var[{index_proc,lru_replacement_proc}];
                            if(lv2_wr_done) begin
                                cache_proc_contr[{index_proc,lru_replacement_proc}]
                                    [CACHE_MESI_MSB:CACHE_MESI_LSB] <= INVALID;
                                addr_bus_lv1_lv2_reg    <= 32'hz;
                                lv2_wr                  <= 1'b0;
                                data_bus_lv1_lv2_reg    <= 32'hz;
                            end
